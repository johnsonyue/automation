...
# Argument parsing
parser = OptionParser()
parser.add_option('-p', '--port', dest='port', type='int', default=12346, help='UDP port to flood')
...
parser.add_option('-s','--seconds', dest='seconds', type='int', default=20, help='Number of seconds to perform the flood')
...
#	print 'Flooding %s:%s for %s seconds: %d byte packets at %s packets/sec' % (options.host, options.port, options.seconds, options.length, rate)
	...
	# Print results
#	print 'Sent %s packets/sec (%.1f mbit/sec)' % ( locale.format('%d', options.counter / options.seconds, 1), options.counter * (42 + options.length) / options.seconds * 8.0 / 1000 / 1000)
